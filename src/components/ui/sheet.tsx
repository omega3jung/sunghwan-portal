...
>(({ className, overlayStyle, ...props }, ref) => (
  <SheetPrimitive.Overlay
    ref={ref}
    className={cn(sheetOverlayVariants({ overlayStyle }), className)}
    {...props}
  />
));
...
const sheetVariants = cva(
  "fixed z-50 bg-background shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    ...
    compoundVariants: [
      // right / left widths
      { side: ["right", "left"], size: "sm", className: "w-3/4 sm:max-w-sm" },
      {
        side: ["right", "left"],
        size: "md",
      ...
      },
      {
        side: ["right", "left"],
        size: "lg",
        className: "w-[32rem] sm:w-[36rem]",
      },
      {
        side: ["right", "left"],
        size: "xl",
        className: "w-[40rem] sm:w-[48rem]",
      },
      {
        side: ["right", "left"],
        size: "full",
      ...
      // top / bottom heights
      { side: ["top", "bottom"], size: "sm", className: "h-auto" },
      { side: ["top", "bottom"], size: "md", className: "max-h-[40vh]" },
      { side: ["top", "bottom"], size: "lg", className: "max-h-[60vh]" },
      { side: ["top", "bottom"], size: "xl", className: "max-h-[80vh]" },
      {
        side: ["top", "bottom"],
        size: "full",
...
> &
  VariantProps<typeof sheetVariants> & {
    overlayStyle?: VariantProps<typeof sheetOverlayVariants>["overlayStyle"];
...
>(
  (
    ...
    <SheetPortal>
      <SheetOverlay overlayStyle={overlayStyle} />
      <SheetPrimitive.Content
        ref={ref}
        className={cn(
          sheetVariants({ side, size, padding }),
          "flex flex-col gap-4",
          className,
        )}
        {...props}
