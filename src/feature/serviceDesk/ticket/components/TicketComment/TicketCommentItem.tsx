...
import { TicketCommentAttachments } from "../TicketAttachmentList";
import { TicketCommentBody } from "./TicketCommentBody";
import { TicketCommentMeta } from "./TicketCommentMeta";
...
export function TicketCommentItem({
  ...
  const files = comment.files.filter((file) => file.active);
  const images = comment.images.filter((image) => image.active);
      ...
      <div className="flex gap-3 p-4">
        <Avatar className="mt-0.5 h-10 w-10 ring-1 ring-border/40">
          ...
          <AvatarFallback className="bg-primary/10 text-primary">
            {initials(owner?.label || comment.ownerId)}
          </AvatarFallback>
        </Avatar>
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <CollapsibleContent className="space-y-4 overflow-hidden data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
            <TicketCommentBody body={comment.body} />
            <TicketCommentAttachments files={files} images={images} />
          </CollapsibleContent>
        </div>
      </div>
    </Collapsible>
