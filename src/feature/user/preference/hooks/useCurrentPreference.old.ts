...
export const useCurrentPreference = (): UseCurrentPreferenceResult => {
  ...
  useEffect(() => {
    ...
    const params = {
      isRemote: session.data.user.dataScope === "REMOTE",
      preferenceKey: preferenceKeys.home.preference,
    };

    const syncPreference = async () => {
      const preference = await userPreferenceRepo.get<PortalPreference>(params);
      ...
      const createdPreference =
        await userPreferenceRepo.create<PortalPreference>({
          ...params,
          data: {
            preferenceKey: preferenceKeys.home.preference,
            preferenceMeta: current,
          },
        });
      ...
      if (createdPreference) {
        store.setPreference(createdPreference.preferenceMeta);
      }
    };
