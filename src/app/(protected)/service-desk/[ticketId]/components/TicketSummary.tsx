...
export function TicketSummary({ ticket, requesterName }: TicketSummaryProps) {
  ...
  return (
    <section className="flex flex-col gap-4 sm:flex-row">
      <aside className="flex shrink-0 gap-2 sm:flex-col sm:gap-3">
        <Avatar className="size-12">
          <AvatarImage src={ticket.requester.image ?? undefined} />
          <AvatarFallback>{initials(requesterName || "")}</AvatarFallback>
        </Avatar>

        <div className="flex size-12 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full bg-cyan-50/70 whitespace-nowrap hover:bg-cyan-100/70">
        ...
        <div className="flex size-12 shrink-0 flex-col items-center justify-center gap-0.5 rounded-full bg-amber-50/70 whitespace-nowrap hover:bg-amber-100/70">
      ...
      <div className="min-w-0 flex-1 space-y-3">
        <h1 className="wrap-break-word text-xl font-semibold leading-tight tracking-[-0.01em] sm:text-2xl">
          {ticket.subject}
        </h1>
