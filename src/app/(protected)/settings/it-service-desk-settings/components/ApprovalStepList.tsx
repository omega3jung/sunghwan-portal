...
export const ApprovalStepList = (props: ApprovalStepListProps) => {
  ...
  const [tableData, setTableData] = useState<DepartmentStepData[]>([]);
  const [stepData, setStepData] = useState<StepData[]>([]);
  const [selectedRow, setSelectedRow] = useState<Department>();
  const [currentStep, setCurrentStep] = useState<number>(0);

  // office department data.
  const departmentData = useMemo<ValueLabel[]>(() => {
    if (!departments) {
      return [];
    }
  ...
  const handleMove = (serviceHubDeptID: number, movedItems: StepData[]) => {
