...
const assignSelfTicket: LocalActionHandler = (context) => {
  ...
  if (previousAssigneeUsernames.length < 2) {
    throw new ApiError(
      "serviceDesk.ticketCommand.localDemo.assignSelfRequiresMultipleAssignees",
      409,
      {
        ticketId: ticket.id,
        username: context.employeeUserName,
...
const adjustTicket: LocalActionHandler = (context) => {
  ...
  if (isAdminCorrection && dueAt !== ticket.due_at) {
    throw new ApiError(
      "serviceDesk.ticketCommand.localDemo.dueAtCorrectionForbidden",
      400,
      { ticketId: ticket.id },
    );
  }
  ...
  if (changedFields.length === 0) {
    throw new ApiError(
      "serviceDesk.ticketCommand.localDemo.noPlanningChanges",
      400,
      { ticketId: ticket.id },
    );
  }
