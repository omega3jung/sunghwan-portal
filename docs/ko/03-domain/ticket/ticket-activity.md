...
- `resubmit`: 반려된 티켓을 `Open`으로 다시 보낸다
- `assignSelf`: 자격 있는 운영자가 빠르게 직접 담당할 수 있게 한다
...
- restriction: content 필수, 작성자만 수정 가능
...
- who: `Working`에서는 assignee, 더 넓은 운영 상태에서는 manager/admin
- when: role과 현재 status에 따라 다르다
- effect: assignee를 갱신하고 필요하면 작업을 다시 활성화한다
...
- who: assignee 또는 manager/admin
- when: 유효한 작업/검토 상태이며, manager는 더 넓은 예외 처리를 가진다
...
- when: `Rejected`
- effect: `Open`
- purpose: approval flow에 다시 진입한다
- restriction: content 필수

### Assign Self

- who: category 또는 job-field rule에 따라 자격이 있는 사용자
- when: `Open`, `Approved`, 또는 `Working`
- effect: assignSelf를 수행하고 필요하면 ticket을 `Working`으로 이동시킨다
- purpose: 빠른 self-assignment
- restriction: 중복 할당을 방지해야 하며, content는 자동 생성된다
...
- `resubmit`은 ticket을 다시 `Open`으로 보낼 수 있다
- manager reassignment는 declined 또는 rejected ticket을 다시 활성화할 수 있다
