...
Full SLA breach, pause, escalation, calendar, notification engine은 future scope입니다.
...
calculation을 completed behavior처럼 설명하면 안 됩니다.

---

## 현재 Input

현재 Service Desk model은 다음 SLA-related input을 가집니다.
...
Category default는 tenant-scoped입니다. Subcategory default가 있으면 main category
default를 override합니다.
...
Requester가 category를 선택할 때 UI는 category default를 적용할 수 있습니다.
...
Ticket form은 `dueAt`이 오늘 이후 날짜인지 validate합니다.

Requester update는 ticket이 다음 상태일 때 due date를 변경할 수 있습니다.
...
현재 requester update policy에서 due date는 routing-neutral입니다.

- due date와 email recipients만 바뀌면 routing을 preserve합니다.
- category, subject, content, files, images가 바뀌면 routing을 reset합니다.
...
days에서 minimum due date도 다시 평가합니다.
...
planning-oriented field를 업데이트할 수 있습니다.

이 변경은 조용한 field mutation이 아니라 event-based history로 기록되어야 합니다.

---

## Work Session과의 관계

Work session은 operational work evidence를 기록합니다. 현재 full SLA clock을 구현하지
않습니다.

Work session은 future SLA reporting이 다음 질문에 답하는 데 도움을 줄 수 있습니다.
...
이는 SLA breach handling과 같지 않습니다.
...
Future production SLA engine은 다음을 추가할 수 있습니다.
...
이 항목들은 현재 due date behavior에 포함된 것처럼 설명하지 말고 explicit service, data field,
history event를 통해 도입해야 합니다.
...
engine을 실행하는 것으로 설명하면 안 됩니다.

### Pending을 구현된 SLA Pause로 취급하기

`Pending`은 workflow status입니다. 현재 fully implemented SLA pause clock이 아닙니다.

### Due Date Change 숨기기

Due date change는 operational planning change이며 traceable해야 합니다.
...
LOCAL과 REMOTE는 동일한 main/subcategory fallback과 서버 권위 규칙을 사용합니다.

## 요약

현재 SLA model은 의도적으로 작고 구현과 정렬되어 있습니다. Category settings는 default
