...
Ticket Action은 사용자에게 보이는 activity timeline과 command record를 설명합니다.

이는 immutable Ticket History와 관련은 있지만 같은 개념은 아닙니다.
...
사용하지만, persisted/action DTO type은 uppercase union을 사용합니다.
...
- 티켓이 `Closed` 된 뒤에도 기존 comment는 계속 표시됩니다.
- 이 visibility가 closure 이후 새 comment를 만들 수 있다는 뜻은 아닙니다.
- `COMMENT_CREATED`를 만듭니다.
- 현재 route는 `Closed` 전 author soft delete를 지원합니다.
- soft delete는 `COMMENT_DELETED`를 만듭니다.
...
update behavior를 노출하지 않습니다.

---

## Operational Actions

Operational action은 ticket state나 routing을 변경할 수 있는 command입니다.
...
Operational action은 normal workflow에서 immutable합니다. 수정은 기존 operational
action row를 편집하는 대신 새 corrective command로 표현해야 합니다.

---

## Command Result

Action command는 다음을 만들 수 있습니다.
...
Action은 audit source of truth가 아닙니다. History가 source of truth입니다.

`actionNo`는 적용 가능한 경우 history를 원인 action과 연결합니다.

System operation은 `actionNo = null`인 history를 만들 수 있습니다. Work-session status
change도 ticket action row 없이 status history를 만들 수 있습니다.

---

## Timeline UI

Ticket timeline은 두 가지를 모두 보여줄 수 있습니다.

- 사람이 읽기 쉬운 communication 및 command reason을 위한 action list
- immutable before/after event를 위한 history list

UI는 모든 history record에 대응 action이 있는 것처럼 보여주면 안 됩니다. 또한 모든
action을 status change처럼 취급해서도 안 됩니다.

---

## Attachment Relationship

Action form은 해당 action이 지원하는 경우 attachment를 포함할 수 있습니다.

Approval action은 file이나 inline image를 허용하지 않습니다.

Action attachment payload는 prepared/safe value를 사용해야 합니다. Blob URL과 data
URL은 action payload validator에서 거부됩니다.
...
Ticket Action은 사용자에게 보이는 activity timeline의 first-class model입니다.
Command와 communication을 기록하고, Ticket History는 immutable audit event를
기록합니다. 둘을 분리하면 UI가 사용자의 행위를 설명하면서도 정확한 event
traceability를 잃지 않습니다.
