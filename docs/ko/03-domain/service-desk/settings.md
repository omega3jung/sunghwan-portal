...
분리를 기준으로 서비스 데스크 설정 설계를 설명하는 것입니다.
...
Route handler는 HTTP 오케스트레이션 경계입니다. 요청을 파싱하고, 세션과 런타임
컨텍스트를 확인한 뒤, 실제 처리를 위임해야 합니다. SQL, mock mutation 규칙,
row-to-DTO 매핑을 route handler가 직접 소유하면 안 됩니다.

LOCAL 설정 동작은 수정 가능한 데모 흐름에서 서버 측 mutable demo state를
사용합니다. REMOTE 동작은 DTO service와 repository를 사용합니다. 두 런타임은
호환 가능한 애플리케이션 응답 형태를 반환해야 합니다.

권한은 LOCAL/REMOTE 분기 전에 결정합니다. 따라서 query filtering, mutation
validation, actor-candidate lookup은 두 runtime에서 같은 권한 결과를 만들어야 합니다.
...
서비스 데스크 설정에는 두 종류의 관리자 principal이 있습니다. 이 값은 client claim이나
role hierarchy가 아니라 서버에서 확인한 canonical `AppUser`로부터 파생합니다.
...
있는 값으로 만듭니다. Impersonation 중 resource capability는 effective user를 따르며,
감사 context에는 original username과 effective username을 모두 유지합니다.

Feature code에서 숫자 값을 반복하지 말고 canonical access-level constant를 사용합니다.
`role`, `dataScope`, focused tenant, request의 `tenantId`/`companyId`, client state는
관리자 유형을 결정하지 않습니다.

Owner Admin과 Tenant Admin은 resource capability가 서로 다른 동등한 관리자입니다.
Owner Admin은 Tenant Admin의 상위 super-role이 아닙니다. 예를 들어 customer
`PORTAL` approval pipeline은 customer Tenant Admin이 관리하고 Owner Admin은
read-only access만 가집니다.

### Tenant 경계와 관리 권한

Tenant는 category 기반 workflow 설정이 _어디에_ 적용되는지를 나타냅니다. 권한 정책은
각 설정 resource를 _누가_ 읽거나 관리할 수 있는지를 정합니다. "tenant가 설정을
소유한다"는 표현은 설정이 tenant 경계에 속한다는 뜻이지, tenant 측 actor 한 명이
그 경계의 모든 resource를 관리한다는 뜻이 아닙니다.
...
Client는 다른 tenant를 선택해 이 경계를 확장할 수 없습니다. Owner Tenant와
owner/service-provider company는 hard-coded ID가 아니라 canonical portal-owner
company resolver 또는 flag로 식별합니다.
...
Owner Admin은 customer `INTERNAL` 설정에 대한 암묵적인 support/read access가 없습니다.
별도의 platform/support capability는 이 정책의 범위에 포함하지 않습니다.

Tenant Admin은 다른 설정 페이지에서 자신의 tenant를 표시하는 데 필요한 최소
metadata를 받을 수 있습니다. 이것이 Tenant Settings 목록이나 mutation API에 대한
access를 부여하지는 않습니다.
...
경계를 정의합니다. 이 경계 안의 관리 권한은 설정 capability matrix에서 별도로
결정합니다.

Tenant는 Company와 관련되지만 같은 개념은 아닙니다. Company는 조직 기준
...
포털 소유자 동작을 파생합니다. 이 보호 동작은 자유롭게 수정 가능한 일반 설정이
아닙니다.
...
한 번에 하나의 focused tenant만 편집합니다. 테넌트 설정 워크플로에서는 여러
테넌트가 선택되거나 활성화될 수 있습니다.

### 활성화 정책

테넌트 활성화는 회사가 서비스 데스크 구성 표면에 참여하는지 제어합니다.

- 테넌트를 활성화하면 설정 구성 대상으로 사용할 수 있습니다.
- 테넌트를 비활성화하면 active 설정 워크플로에서 제외됩니다.
- 포털 소유자 보호 동작은 서버 경계에서 강제해야 합니다.
- 테넌트가 나중에 비활성화되어도 기존 티켓은 읽을 수 있어야 합니다.

Tenant Settings 목록과 관리는 Owner Admin에게만 허용합니다. Tenant Admin에게는
Tenant Settings tab을 표시하지 않으며 직접 page에 접근하면 UI guard로 Settings
Home으로 redirect합니다. 권한 없는 Tenant Settings API 요청은 `403`을 반환하며,
API 권한 처리는 redirect를 사용하지 않습니다.
...
현재 도메인 모델은 main category와 subcategory를 구분합니다.
...
Main category는 필수 기본값을 가집니다. Subcategory는 일부 기본값을 덮어쓸 수
있습니다. Subcategory에 기본값이 없으면 main category 값이 fallback으로
사용됩니다.
...
Scope는 main category에 속합니다. Subcategory는 표시와 라우팅 목적에서 main
category의 scope를 상속합니다.

### Active 정책

Category 생성과 활성화는 서로 다른 lifecycle 단계입니다.
...
main category와 새 subcategory를 모두 `active = false`로 강제합니다. 비활성
category도 Settings에서는 표시하고 구성할 수 있지만, 새 ticket 선택지로는
제공하지 않습니다.

`inactive -> active` 전환에는 현재 active인 Job Field reference 또는 active인
Employee reference를 하나 이상 포함한 effective Assignment Rule이 필요합니다. Main
category의 effective rule은 자신의 rule입니다. Subcategory는 own rule이 있으면 이를
우선 사용하고, own rule이 없을 때만 main-category rule을 사용합니다. Own rule이
존재하지만 유효하지 않은 경우 parent로 fallback하지 않습니다.

이 activation 검증은 configuration readiness 검사이며 최종 routing 검증이 아닙니다.
...
Activation 시점에는 Job Field를 현재 employee로 확장하지 않습니다. Ticket submit,
...
Main category와 subcategory의 stored state는 서로 독립적입니다. Main category를
비활성화해도 child state를 덮어쓰지 않습니다.

```ts
const effectiveActive = mainCategory.active && subCategory.active;
```

따라서 main category를 다시 활성화하면 각 child의 이전 effective state가 복원됩니다.
이미 비활성 category를 참조하는 기존 ticket은 계속 읽을 수 있고 감사 가능해야 합니다.
...
Main category 생성은 capability matrix를 따릅니다.

- Owner Admin은 Owner Tenant에 `INTERNAL`, `PORTAL` category를 만들 수 있습니다.
- Owner Admin은 customer Tenant에 `PORTAL` category만 만들 수 있습니다.
- customer Tenant Admin은 자신의 Tenant에 `INTERNAL` category만 만들 수 있습니다.
...
Scope 변경은 기존 category를 비활성화하고 새 category를 만드는 방식으로 모델링합니다.
Category는 hard delete 대신 `active = false`를 사용하여 기존 ticket과 history의
category 참조를 보존합니다. Subcategory는 parent main category의 tenant와 scope를
상속하며 독립된 scope capability를 갖지 않습니다.

Update 또는 deactivate 시 서버는 저장된 category를 먼저 로드한 다음 권한을
확인합니다. Create 시에는 대상 tenant와 요청 scope를 검증합니다. Subcategory는 parent
main category를 로드해 관계로부터 tenant/scope를 파생합니다. Request payload field를
권한 사실로 신뢰하지 않습니다.
...
승인 단계는 main category의 순서 있는 승인 동작을 정의합니다.
...
resolve됩니다. Subcategory는 요청을 classify하지만 독립적인 approval pipeline을 만들지
않습니다.
...
티켓 row는 현재 책임자를 ticket assignment field에 저장합니다. 과거 승인 구성
...
결정된 모든 approver는 category tenant의 company에 속해야 합니다.

- `EMPLOYEE`: 모든 employee의 `companyId`가 category tenant와 같아야 합니다.
- `DEPARTMENT`: department와 결정된 employee가 해당 company 안에 있어야 합니다.
- `JOB_FIELD`: job field가 공유 기준 데이터여도 최종 employee 결정에 company filter를 적용합니다.
- `MANAGER`: 결정된 manager가 해당 company에 속해야 합니다.

Eligibility는 두 경계에서 검사합니다. Candidate read API는 현재 principal이 선택할 수
있는 reference만 반환합니다. REMOTE settings write는 candidate list를 다시 조회하지
않고 PostgreSQL이 저장된 category, tenant, company, active organization row를 기준으로
제출된 approval reference 전체를 tree 저장 transaction 안에서 검증합니다. Ticket
submit/resubmit 및 명시적 routing 재계산은 설정 이후 organization data가 바뀔 수
있으므로 eligibility를 다시 검사합니다. 유효한 approver가 없으면 routing은 실패하며
owner 없는 `Approval` ticket을 만들지 않습니다.

Customer `PORTAL` approval 설정에 read-only access가 있는 Owner Admin에게는 현재
참조된 approver의 표시 데이터를 제공할 수 있습니다. 이것이 customer employee directory의
candidate search 권한을 부여하지는 않습니다.
...
작업 담당자를 해석해야 할 때 work assignee를 결정합니다.
...
현재 모델은 group 기반입니다. 별도의 `ruleType` 필드를 사용하지 않습니다. REMOTE
DTO는 `job_field_id`와 `employee_username` 배열을 사용하며, 다시
애플리케이션 모델로 매핑됩니다.

### 해석

배정 해석은 서버 경계에서 이루어져야 합니다.
...
빈 form state는 persisted Assignment Rule이 아닙니다. Rule은
...
빈 override를 저장하지 않습니다. Configuration이 명시적 activation보다 먼저 이루어져야
하므로 main category가 inactive인 상태에서도 rule을 구성할 수 있습니다.

Assignment candidate, 저장 검증, activation readiness, recommendation, routing은 저장된
category에서 파생한 하나의 정책을 사용합니다. Owner `INTERNAL`은 owner company,
customer `INTERNAL`은 Tenant company, 기본 `PORTAL`은 owner company,
`includeTenantCompany`가 설정된 `PORTAL`은 두 company를 모두 사용합니다. Employee와
Job Field reference는 이 eligible company 집합에 속해야 합니다. Client가 제공한 category
scope, owner flag, 미리 계산한 company list는 authoritative source가 아닙니다.

Candidate read API도 같은 company 집합을 파생한 뒤 department, job field, employee를
반환합니다. REMOTE save에서는 PostgreSQL이 저장된
category로부터 canonical policy를 결정하고 제출된 job-field 및 employee reference
전체를 assignment-tree write transaction 안에서 set-based query로 검증합니다. Active
Job Field는 현재 active employee가 없더라도 유효한 configuration reference이며, 이를
실제 worker로 확장하는 것은 routing-time validation의 책임입니다. Submit, resubmit,
명시적 routing command는 설정 이후 organization data가 바뀔 수 있으므로 eligibility를
다시 검사합니다. 유효한 worker가 없으면 routing은 owner 없는 `Assigned` ticket을 만드는
대신 실패합니다.

Customer `PORTAL` assignment rule에 read-only access가 있는 Tenant Admin에게는 현재
참조된 provider assignee의 표시 데이터를 제공할 수 있습니다. 이것이 owner-company
employee directory 전체의 candidate search 권한을 부여하지는 않습니다.

배정 규칙 변경은 이후 라우팅 결정에 영향을 줍니다. 기존 티켓은 현재 assignee와
기록된 이력을 유지합니다.
...
Department 데이터는 승인 단계 assignee 구성에 사용됩니다.
...
Organization lookup은 company 중심입니다.
...
설정 page는 선택되고 권한이 확인된 Tenant에서 `companyId`를 얻습니다. Repository
variant는 organization 전체를 읽어 application code에서 filter하지 않고 SQL에
company predicate를 적용합니다. Approval Step과 Assignment Rule은 독립적인 권한
source로 `tenantId`를 저장하지 않습니다. DTO는 파생 context를 project할 수 있습니다.
...
Tenant와 category는 구성 계층을 만듭니다. Approval과 assignment 설정은
category에 연결됩니다. Category가 티켓 워크플로의 주된 동작 단위이기 때문입니다.

Approval resolution은 main-category approval step을 사용합니다. Assignment resolution은
subcategory override를 먼저 사용하고 필요할 때만 parent/main fallback을 사용합니다.
...
새 티켓은 갱신된 category 구성을 사용합니다. 기존 티켓은 저장된 category,
priority, risk, due date, assignee, activity, history를 유지합니다.
...
새 배정 해석은 갱신된 규칙을 사용합니다. 기존 티켓은 티켓 command가 변경하기
전까지 현재 work assignee를 유지합니다.

### Requester Update

Requester update는 작업 실행 전 category와 content를 변경할 수 있습니다.
...
설정 server data를 Zustand에 또 다른 source of truth로 중복 저장하면 안 됩니다.
...
assignment rule은 category별로 표시됩니다.

Read-only view는 "서비스 제공자가 관리", "고객사가 관리", "읽기 전용"과 같이
책임 주체를 표시합니다. Tab이나 button을 숨기는 것은 usability 동작일 뿐이며 서버
권한 검사를 대신하지 않습니다.
...
숨길 것이라고 가정하면 안 됩니다. Mutation API는 저장된 resource context를 다시
로드하고 `manage` access가 없는 principal에게 `403`을 반환합니다. DTO validation은
...
server service/repository에서 다시 검사합니다. 기존 RLS, database function, constraint는
defense in depth로 같은 tenant boundary를 보존해야 합니다.
...
다음 항목은 명시적으로 구현되기 전까지 지연 범위입니다.
...
- 진행 중인 Ticket이 main/subcategory를 사용 중인 Category를 비활성화할 때는
  영향 경고와 명시적 확인이 필요합니다. 확인 후 비활성화해도 기존 Ticket 상태,
  routing, History는 변경하지 않고 신규 workflow availability만 변경합니다.
...
- `Approval` 상태 Ticket에 영향을 주는 Approval Step tree 변경은 force apply가
  필요합니다. Force apply는 새 설정과 모든 재라우팅을 검증한 뒤 설정 저장,
  첫 단계부터의 routing reset, reason이
  `APPROVAL_CONFIGURATION_CHANGED`인 `ROUTING_RESET` History 기록을 하나의
  transaction으로 처리합니다. 하나라도 실패하면 전체를 rollback합니다.
- 이미 진행 중인 approval은 Category가 이후 비활성화되어도 계속할 수 있습니다.
  신규, 재제출, 명시적으로 재시작하는 routing은 operational Category를 요구합니다.
...
rule을 사용합니다. Tenant는 workflow 경계이며 category-scope 권한이 각 resource의
실제 관리자를 결정합니다. `INTERNAL` 설정은 해당 tenant의 관리자가 담당하고,
...
경계를 통해 제공됩니다. 설정 변경은 이후 워크플로를 안내해야 하고, 기존 티켓
activity와 history는 이미 일어난 일의 의미를 보존해야 합니다.
