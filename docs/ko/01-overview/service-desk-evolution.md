...
Service Desk 도메인으로 어떻게 정제되었는지 설명합니다.
...
현재 프로젝트는 그 시스템을 그대로 복제하지 않습니다. 실제 운영에서 얻은 교훈을 바탕으로 더 명확한
Service Desk 도메인으로 재설계합니다.
...
아니라 집중된 workflow domain이기 때문입니다.
...
이전 모델은 상태 업데이트가 있는 request record에 가까웠습니다.

현재 모델은 ticket을 controlled state를 가진 workflow entity로 다룹니다.
...
나올 수 있지만 현재 ticket status가 아닙니다.

---

## Text Update에서 Command로

이전 방식은 comment와 status edit에 많이 의존했습니다.

현재 설계는 명시적인 Ticket Action command를 사용합니다.
...
Comment는 communication입니다. Assignment, rejection, merge, reopen, planning
adjustment는 rule을 가진 command입니다.

---

## Comment와 Note 분리

Communication은 두 개념으로 분리되었습니다.
...
History는 event-based이며 다음을 기록합니다.
...
이는 timeline을 단순 comment list나 free-form `metadata.event`에 의존하지 않게 합니다.

---

## Tenant-Scoped Settings

Settings는 admin CRUD에서 behavior-defining configuration으로 진화했습니다.
...
- `Tenant`는 Service Desk configuration boundary입니다.
- Category scope는 `"PORTAL"` 또는 `"INTERNAL"`입니다.
- Approval step은 ordered, category-based입니다.
- Assignment rule은 job field와 employee username 기반 group model입니다.
- Settings 변경은 미래 behavior에 영향을 주며 과거 history를 rewrite하지 않습니다.

---

## Category-Driven Behavior

Category는 주요 behavior driver입니다.
...
settings의 anchor 역할을 합니다.

Requester category change는 routing-sensitive이며 routing reset을 유발할 수 있습니다.

---

## Approval과 Work Routing

Routing은 phase-aware가 되었습니다.

```ts
type TicketAssignmentPhase = "APPROVAL" | "WORK";
```

Ticket DTO는 다음 projection을 노출할 수 있습니다.
...
모든 assignee를 같은 ownership으로 취급하지 않는 것이 핵심 개선입니다.

---

## Requester Update Policy

요청자는 active work 전 단계에서 제한적으로 수정할 수 있습니다.
...
Create workflow는 one-shot form에서 draft-aware workflow로 발전했습니다.

REMOTE draft는 requester당 하나의 active draft를 가진 `Draft` ticket row로 구현됩니다.

Attachment는 prepare boundary로 발전했습니다.
...
storage를 제공한다고 주장하지 않습니다.

---

## Work Session

Work tracking은 단일 누적 시간에서 work session으로 발전했습니다.
...
현재 route surface는 list/create 중심입니다. Timer/update/delete API는 matching route
handler가 생길 때까지 extension point입니다.

---

## SLA 진화

현재 SLA는 신중하게 범위를 제한합니다.
...
escalation은 future production scope입니다.
...
Assignment, rejection, merge, planning change는 comment가 아니라 command로 표현합니다.

### Settings는 Behavior를 정의

Tenant, category, approval, assignment settings는 future ticket execution을 형성합니다.

### History는 Change를 설명

History는 무엇이, 왜, 누가, 언제 바꿨는지 설명해야 합니다.

### Current Docs와 Decision Logs의 역할 분리

Current design docs는 최신 구현 정렬 모델을 설명합니다. Decision logs는 특정 시점의
맥락과 이유를 보존합니다.
...
Service Desk는 기본 request tracking에서 workflow-centered domain으로 진화했습니다.
현재 모델은 lifecycle, routing, command, history, settings, draft, attachment,
work evidence를 명시하고, future production infrastructure와 현재 구현을 분리합니다.
