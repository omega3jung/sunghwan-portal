...
### 7.5 Service Desk Settings DTO/API 경계

6월 settings 구현은 Service Desk settings를 명시적인 tenant와 DTO/API boundary
중심으로 정렬했다.
...
Service Desk settings API는 LOCAL과 REMOTE mode에서 같은 application-facing contract를
유지해야 한다. UI는 DTO를 소비해야 하며, settings data가 server-side local mock
state에서 왔는지 PostgreSQL-backed repository에서 왔는지 알 필요가 없어야 한다.

구현 가이드:

- route handler는 thin하게 유지한다.
- settings behavior는 domain handler로 위임한다.
- REMOTE data access에서는 row / mapper / DTO boundary를 유지한다.
- LOCAL settings mutation은 server-side local state module에 둔다.
- 실제 UI workflow를 지원하지 않는 speculative CRUD route는 유지하지 않는다.
...
- Service Desk settings를 tenant-scoped DTO/API contract로 정렬
