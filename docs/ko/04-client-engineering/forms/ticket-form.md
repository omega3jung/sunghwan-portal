...
워크플로 결정은 폼이 아니라 서버 경계에서 수행합니다.

현재 구현 기준:

- `CreateTicketDialog`는 신규 티켓 제출과 draft 복구를 담당합니다.
- `UpdateTicketDialog`는 작업 시작 전 요청자 수정을 담당합니다.
- React Hook Form은 다이얼로그가 열려 있는 동안의 미저장 입력을 소유합니다.
- Attachment Prepare API가 브라우저 파일과 본문 이미지를 준비된 metadata로 변환합니다.
- 티켓 서비스가 승인, 할당, 라우팅, 이력을 결정합니다.
...
`CreateTicketDialog`는 요청자용 생성 워크플로입니다.
...
현재 구현은 별도 category-only 단계나 전역 generic stepper를 사용하지 않습니다.
...
- `dueAt`은 오늘 이후여야 합니다.
- `attachment`는 prepare 전까지 브라우저 `File[]`입니다.
- 최종 ticket mutation은 준비된 첨부 metadata를 다시 검증합니다.

---

## 카테고리 기본값

카테고리 선택은 다음 기본값을 seed할 수 있습니다.
...
티켓 쓰기 명령에는 raw `File`이 아니라 준비된 본문과 첨부 metadata만 전달합니다.
...
Draft는 폼 데이터 복구에 초점을 두며, 첨부 복구는 보장하지 않습니다.
...
`ASSIGNMENT_RESOLVED` 같은 event-based history를 기록합니다.
...
Zustand는 form input, draft, attachment metadata의 source of truth가 아닙니다.
...
않습니다. 생성과 수정은 필드를 공유해도 draft, submit, reset, routing 동작이 다릅니다.
...
티켓 폼은 워크플로 권위자가 아니라 워크플로 진입점입니다. 현재 설계는
`CreateTicketDialog`, `UpdateTicketDialog`, 브라우저 로컬 LOCAL 초안 복구,
티켓 초안 API를 통한 REMOTE 초안 복구, 영속화 전 첨부파일 준비, 서버 소유의
요청자 업데이트 라우팅을 사용합니다. 이를 통해 UI 사용성을 유지하면서도 승인,
할당, 상태, 이력의 source of truth는 티켓 서비스에 둡니다.
