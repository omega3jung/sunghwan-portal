...
이 문서는 **Next.js 14 App Router**로 구축한 **Service Desk 시스템**의 설계, 아키텍처, 그리고 구현 관점을 설명한다.

이 프로젝트는 단순한 UI 데모가 아니라, 다음 요소를 중심으로 한 **도메인 주도적이고 실제 운영 환경을 고려한 시스템**으로 문서화되어 있다.
...
- 프로젝트 구조를 검토하는 엔지니어
- 설계 의사결정을 평가하는 인터뷰어
- 실제 프론트엔드 아키텍처에 관심 있는 개발자

구현 내용뿐 아니라, 주요 의사결정의 배경까지 설명하는 것을 목표로 한다.
...
- `03-domain`: Service Desk 도메인 규칙과 워크플로우 설계
...
프로젝트 문서는 더 깊은 시스템 설계로 들어가기 전에, 리포지토리가 스스로를 어떻게 소개해야 하는지 설명한다.
...
아키텍처 문서는 시스템이 애플리케이션 수준에서 어떻게 구성되는지 설명한다.
...
도메인 문서는 Service Desk가 시스템으로서 어떻게 동작하는지 정의한다.

다음 내용을 다룬다.

- Ticket lifecycle과 ownership
- Category 계층과 설정
- 승인 워크플로우와 assignment 규칙
- SLA 로직과 history model
...
이 섹션은 프로젝트가 React Query를 어떻게 사용하는지, 그리고 server/client 책임을 어디에서 분리하는지 설명한다.
...
이 섹션은 시스템에서 가장 도메인 의존성이 높은 상호작용 중 하나인 ticket form에 초점을 둔다.
...
i18n 문서는 번역 파일이 어떻게 구성되는지, 그리고 validation feedback이 메시지 책임에 따라 어떻게 분리되는지 설명한다.
...
개발 전략 문서는 프로젝트가 실제로 어떻게 만들어지고 발전했는지 설명한다.
...
이 구조는 완전히 사전 설계된 시스템이 아니라, **반복적이고 현실적인 개발 과정**을 반영한다.
...
- 시스템 핵심 동작으로서의 **추적 가능성과 감사 가능성**
- 획일적인 화면 대신 **역할 인식형 UX**
- 성급한 추상화보다 **실용적인 반복 개선**
...
프로젝트를 처음 본다면, 다음 순서가 가장 명확한 개요를 제공한다.
...
이 문서는 단지 **무엇을 만들었는지**만이 아니라, **왜 그런 설계 결정을 내렸는지**까지 보여주기 위해 작성되었다.

이 프로젝트를 다음 특징을 가진 시스템으로 제시한다.
...
`docs/ko`는 **프로젝트 문서 전략부터 아키텍처, 도메인 동작, UI/UX, 데이터 전략, 폼, 로컬라이제이션, 개발 의사결정**까지 아우르는 Service Desk 시스템 개요다.

이 문서들을 함께 보면, 이 프로젝트가 단순히 시각적으로 완성된 결과물이 아니라 **실용적이고, 추적 가능하며, 확장 가능하고, 실제 Service Desk 워크플로우에 기반한 시스템**임을 보여준다.

가장 짧은 경로로 문서를 읽고 싶다면 이 파일부터 시작한 뒤, [README Strategy](./01-project/readme-strategy.md)로 이동하고, 위의 권장 읽기 순서를 따라가면 된다.
