...
문서는 구현 내용뿐 아니라, 핵심 설계 결정을 왜 내렸는지까지 설명하는 것을 목표로 합니다.

---

## 이 문서가 다루는 내용

`docs/ko` 폴더는 프로젝트의 주요 설계 축을 기준으로 구성되어 있습니다.
...
- `07-i18n`: localization 구조와 validation messaging
...
저장소가 스스로를 어떻게 소개해야 하는지를 설명합니다.

다음 내용을 다룹니다.

- README 구조와 목적
- 언어 분리 전략
- 저장소 진입점에서 상세 문서로 이어지는 탐색 흐름
...
아키텍처 문서는 시스템이 애플리케이션 수준에서 어떻게 구성되는지를 설명합니다.
...
도메인 문서는 Service Desk가 워크플로 중심 시스템으로서 어떻게 동작하는지를 정의합니다.
...
UI/UX 문서는 시스템 동작이 어떻게 사용 가능한 인터페이스로 번역되는지에 초점을 둡니다.
...
이 섹션은 프로젝트가 React Query를 어떻게 사용하고,
server/client 책임을 어디에서 분리하는지를 설명합니다.
...
그리고 validation feedback이 메시지 책임에 따라 어떻게 분리되는지 설명합니다.
...
개발 전략 문서는 프로젝트가 실제로 어떻게 만들어지고 발전했는지를 설명합니다.
...
`decision-log` 디렉터리는 개발 과정에서 내려진 중요한 설계 결정을 기록합니다.
...
이 구조는 완전히 사전 설계된 시스템이 아니라
**반복적이고 실제적인 개발 과정**을 반영합니다.

현재 의사결정 로그 주제는 다음과 같습니다.
...
모든 섹션에서 이 문서는 다음 주제를 일관되게 반영합니다.

- 일반적인 CRUD 사고보다 **도메인 우선 설계**
...
프로젝트를 처음 보는 경우, 다음 순서가 가장 명확한 개요를 제공합니다.
...
이 문서는 단지 **무엇을 만들었는지**뿐 아니라
**왜 특정한 설계 결정을 내렸는지**까지 보여주기 위해 작성되었습니다.

프로젝트를 다음 특성을 가진 시스템으로 제시합니다.
...
독자는 이 문서를 통해 다음 질문에 빠르게 답할 수 있어야 합니다.

- 이 프로젝트는 어떤 종류의 시스템인가?
- 각 주제는 어디에 문서화되어 있는가?
- 어떤 문서가 안정적인 설계를 설명하고, 어떤 문서가 반복적인 구현 과정의 결정을 설명하는가?

---

## 요약

`docs/ko`는 **프로젝트 문서 전략부터 아키텍처, 도메인 동작, UI/UX, 데이터 전략, 폼, 로컬라이제이션, 개발 의사결정**까지를 아우르는 Service Desk 시스템 개요입니다.

이 문서들을 함께 보면, 이 프로젝트가 단순히 시각적으로 완성된 결과물이 아니라
**실용적이고, 추적 가능하며, 확장 가능하고, 실제 Service Desk 워크플로에 기반한 시스템**임을 보여줍니다.
