...
- 대신 행동하는 대상 사용자
- impersonation 동안 시스템에서 실제로 사용되는 effective identity
...
관리자(original) -> impersonates -> 직원(effective)
```
...
- `user` 는 UI와 API 전반에서 effective user로 사용됨
...
- effective user가 바뀌더라도 original actor는 항상 식별 가능해야 함
...
- 시스템은 여전히 원래 actor가 관리자임을 알고 있음
