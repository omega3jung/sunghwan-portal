...
`sessionStorage` 기반의 초기 impersonation 구현은 다음과 같은 치명적인 한계를 드러냈습니다.
...
**프로덕션에 맞는 안전하고 일관된 시스템**으로 발전시키는 것이 목표였습니다.
...
Impersonation을 **클라이언트 상태**에서 **NextAuth 기반 서버 제어 세션 모델**로 옮기기로 했습니다.
...
- UI, API, 서버 전반에서 신원 정보를 일관되게 유지할 수 있습니다
- 프런트엔드와 백엔드 사이의 불일치를 제거합니다
- 런타임 동작을 더 예측 가능하게 만듭니다

---

### 2. Security

- 클라이언트에서 쉽게 조작할 수 없습니다
- 인증된 세션을 통해 통제됩니다
- 역할 기반 제한을 중앙에서 강제할 수 있습니다

---

### 3. SSR Compatibility

- Next.js App Router와 함께 동작합니다
- 서버 컴포넌트와 route handler에서 사용할 수 있습니다
...
- 기존 인증 시스템에 대한 영향이 최소화됩니다
- 신원 컨텍스트를 중앙에서 제어할 수 있습니다
- 현재 인증 모델과 더 깔끔하게 통합됩니다
...
- 액션은 `currentUser` 기준으로 수행됩니다
- 시스템은 여전히 `originalUser`를 고려하여 권한을 검증합니다
- 원래 행위자는 흐름 전체에서 추적 가능하게 남습니다

---

### Example

- 관리자가 사용자를 impersonation합니다
- 액션은 그 사용자 컨텍스트에서 수행됩니다
- 시스템은 원래 행위자가 관리자라는 사실을 여전히 알고 있습니다
...
Impersonation이 활성화되면 UI는 다음을 만족해야 합니다.

- impersonation 배너를 표시합니다
- 가장 중인 사용자 신원을 명확하게 보여줍니다
- 눈에 잘 띄는 `Stop Impersonation` 액션을 제공합니다
...
- 혼란을 방지합니다
- 투명성을 높입니다
- 잘못된 신원 아래에서 의도하지 않은 작업이 일어나는 것을 줄입니다
...
- 서버는 세션을 읽습니다
- 데이터 접근은 `currentUser`를 사용합니다
- 감사 로그에는 `originalUser`를 저장합니다
...
**서버가 제어하는 세션 모델**로 발전했습니다.

이를 통해 애플리케이션 전반에서 안전하고 일관되며,
완전히 추적 가능한 사용자 컨텍스트 전환이 가능해졌습니다.
