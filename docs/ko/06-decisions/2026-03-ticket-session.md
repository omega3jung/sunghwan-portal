...
실제 운영 워크플로에 맞는 방식으로 티켓별 시간 추적을 지원할 필요가 생겼습니다.

시스템은 다음을 처리할 수 있어야 했습니다.
...
그리고 작업 컨텍스트 기반 UX와 관련된 설계 결정을 정리합니다.
...
- 실제 작업은 하나의 티켓에서도 여러 세션에 걸쳐 발생하는 경우가 많습니다
- 모델이 중단, 재할당, 작업 재개를 지원할 수 있어야 합니다
- Jira worklog 같은 엔터프라이즈 도구와 더 잘 맞습니다
...
- `startAt`은 항상 존재합니다
- `endAt === null`이면 현재 실행 중인 세션을 의미합니다
- `durationMinutes`는 세션 종료 시점에 결정됩니다
- 하나의 티켓에 여러 세션이 존재할 수 있습니다
...
별도의 `TicketTrackLog` 테이블은 추가하지 않기로 했습니다.

### 이유

- 이미 `TicketTrackTime`이 표현하는 정보를 중복하게 됩니다
- 테이블 간 정합성 위험이 커집니다
- 얻는 가치에 비해 복잡도만 증가합니다
...
시작과 종료는 폼 입력이 아니라 직접적인 API 액션으로 구현하기로 했습니다.

### 이유

- 실제 시간 값은 서버가 제어해야 합니다
- 단일 액션 방식이 UI에서 더 빠릅니다
- 클라이언트 측 시간 조작을 피할 수 있습니다
...
- `start`는 `startAt = now`인 새 세션을 만듭니다
- `finish`는 `endAt = now`로 현재 활성 세션을 종료합니다
- `durationMinutes`는 종료 시 계산됩니다
...
티켓 간 자연스러운 이동을 지원하기 위해 전용 `switch` API를 추가하기로 했습니다.
...
1. 실행 중인 세션이 있으면 먼저 종료합니다
2. 대상 티켓에 대한 새 세션을 시작합니다
3. 새로 생성된 세션을 반환합니다
...
수동 시간 입력은 두 가지 폼 모드로 분리하기로 했습니다.
...
- 사용자는 시간을 서로 다른 방식으로 인식합니다
- 모드를 나누면 검증이 더 단순해집니다
- 지나치게 복잡한 조건부 스키마를 피할 수 있습니다

### 규칙

- 하나의 폼에서는 하나의 입력 모드만 사용합니다
- 시간 구간 입력을 사용할 때 `durationMinutes`는 서버가 계산합니다
...
`endAt IS NULL`인 세션은 하나만 존재할 수 있습니다.
...
- 어떤 세션이 활성 상태인지 항상 명확합니다
- 타이머 동작을 더 쉽게 추론할 수 있습니다
...
현재 사용자의 작업 상태에 따라 티켓을 정렬하기로 했습니다.
...
- 티켓 목록이 작업 중심 inbox처럼 동작해야 합니다
- 바로 처리할 수 있는 항목이 먼저 보여야 합니다
- 일상적인 운영 흐름을 더 잘 지원할 수 있습니다
...
- Finish와 Switch 같은 조건부 UI를 지원할 수 있습니다
- 타이머 상태를 별도의 전역 스토어에 중복 저장하지 않아도 됩니다
- UI를 쿼리나 서버 데이터에서 파생된 상태로 유지할 수 있습니다
...
이 설계는 다음 요소를 가진 세션 기반 시간 추적 모델을 확립합니다.
