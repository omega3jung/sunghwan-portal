...
Category 시스템은 Service Desk 시스템의 중앙 설정 계층입니다.
...
초기 category 설계는 다음과 같은 **계층 구조**를 중심으로 구성되었습니다.

```txt
Client -> MainCategory -> SubCategory
```

그리고 `SubCategory`가 `MainCategory`의 값을 재정의하도록 설계되었습니다.
...
이 결정은 category-driven 워크플로 원칙을 유지합니다.
...
- category에서 파생된 기본값은 계속 보이도록 해야 합니다
- override된 값은 기본값과 구분 가능해야 합니다
...
이 결정은 시스템의 세 가지 성질 사이의 균형을 맞춥니다.

### Consistency

- category가 여전히 기본 워크플로를 결정합니다
- 티켓의 핵심 동작이 예측 가능하게 유지됩니다

---

### Flexibility

- 현실적인 예외 상황을 처리할 수 있습니다
- 운영자가 지나치게 경직된 설정 때문에 막히지 않습니다
...
- `category-strategy.md`에 구조적 변경은 필요하지 않습니다
- 현재 category 모델은 개념적으로 이미 이 방향을 지원하고 있습니다

---

### Ticket 시스템에 대한 영향

- 티켓 모델은 override 필드를 지원해야 합니다
- 히스토리 추적이 핵심 인프라가 됩니다
...
시스템은 **category-driven이지만 override가 가능한 모델**을 채택했습니다.

- Category가 기본 동작을 정의합니다
- Ticket은 통제된 override를 허용합니다
- 모든 override는 명시적이어야 하며 추적 가능해야 합니다

이를 통해 시스템은 다음 특성을 유지합니다.
